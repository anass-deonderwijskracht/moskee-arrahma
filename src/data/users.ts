...
/** All admins + docenten, with the docent's linked class code. Admin-gated by RLS. */
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AppUser[]> => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, full_name, email, role, class_id, classes(code)")
        .order("role", { ascending: true })
        .order("full_name", { ascending: true });
      if (error) throw error;
      type Row = { id: string; full_name: string | null; email: string | null; role: string; class_id: string | null; classes: { code: string } | null };
